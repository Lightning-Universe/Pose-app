...
def set_script_args(*args, **kwargs):
  script_args_dict = args_utils.args_to_dict(st.session_state['script_args'])
  ...
  st.session_state['script_args'] = args_utils.dict_to_args(script_args_dict)
...
def run(root_dir=".", context=st):
    ...
    st.text_area("Script Args", placeholder='Use K=V format. Use a=1 --b=2 NOT a 1 --b 2', on_change = set_script_args, key="script_args", kwargs={'key':'script_args'})

    st.text_area("Script Env", placeholder='a=1 b=2', on_change = set_script_args, key="script_env", kwargs={'key':'script_env'})
