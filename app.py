...
from pose_app import *

class App(LightningFlow):
  def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    self.ui_train = TrainUI()
    self.work_train = TrainScript()
    self.work_tb = CreateTensorboard()
    self.ui_fo = CreateFiftyoneUI()
    self.work_fo = CreateFiftyoneDataset()

  def run(self):
    self.ui_train.run()
    self.ui_tb.run()
    # train
    self.ui_train.st_submit:
