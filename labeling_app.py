...
class LitPoseApp(LightningFlow):
    ...
    def run(self):
        ...
        # -------------------------------------------------------------
        if (self.extract_ui.proj_dir and (
            self.extract_ui.run_script_video_random or self.extract_ui.run_script_zipped_frames
            )
        ):
            if self.extract_ui.run_script_video_random:
                self.extract_ui.run(
                    action="extract_frames",
                    video_files=self.extract_ui.st_video_files,  # add arg for run caching purposes
                )
            elif self.extract_ui.run_script_zipped_frames:
                self.extract_ui.run(
                    action="unzip_frames",
                    video_files=self.extract_ui.st_video_files,  # add arg for run caching purposes
                )
            ...
            if self.extract_ui.work_is_done_extract_frames:
                ...
                self.extract_ui.run_script_video_random = False  # app won't advance past ls run
